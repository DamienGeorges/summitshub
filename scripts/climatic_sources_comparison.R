...
#               overwrite = TRUE)
# })
# xop.dat <- stack(file.path(xop.out.dir, paste0("xop_", row.to.consider, ".grd")))
# names(xop.dat) <- paste0("xop_", row.to.consider)
# rm(xop.tab, xop.grd)
...
# ## produce individual rasters
# cas.tab <- readLines("../DATA/climate/casty/temp-mon.txt", skipNul = TRUE)
...
unique.xop.id <- s.dat.all.cells$unique_id[which(!duplicated(s.dat.all.cells$xoplakis_cells))]
