...
##' @log
##'   - 2017-05-12: change the alti correction coef for - 0.6°/100m
##' @licence GPL 2.
##' ----------------------------------------------------------------------------
...
dat.ref <- dat.ref %>% mutate(delta_wc_alti = (mtn_altitude - wc_alt) * (- 0.6 / 100))
...
delta.wc.dat <- dat.comp %>% dplyr::select(s1Cell, s1, s2, period, delta) %>% filter(s1 == 'wc') %>%
  ...
  dplyr::select(unique_id, clim_source, period, delta_corrected)
...
tmp.dwsc <- tmp.dwsc %>% 
  ...
  dplyr::select(mountain_id, mountain_name, ycoord, xcoord, mtn_altitude, clim_source, year, period, mean_tmp_dwsc)
